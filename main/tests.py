...
class Tests(APITestCase):
    ...
    def test_posts(self):
        response_0 = self.guest_client.get(POST_URL)
        self.assertEqual(
        ...
        response1 = self.authorized_client.post(self.POST_READ_URL)
        self.assertEqual(response1.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response1.data.get('post'), self.post.pk)
        self.assertEqual(response1.data.get('user'), self.user.pk)
        response2 = self.authorized_client.get(POST_LIST_FILTERED_BY_READ_URL)
        self.assertEqual(response2.status_code, status.HTTP_200_OK)
        read_content = dict(*response2.data.get('results'))
